...
class Dreamobjects extends Base
{
	public $cache = null;
	...
	public function processPart($absolute_filename, $upload_as = null)
	{
		...
		if (empty($directory))
		{
			$directory = $config->get('engine.postproc.dreamobjects.directory', 0);
		}
		...
		Factory::getLog()->log(LogLevel::DEBUG, "DreamObjects -- Uploading " . basename($absolute_filename));
		...
		$result = $s3->putObject(
			...
			ConnectorAmazons3::ACL_PRIVATE, // ACL (bucket owner has full control, file owner gets full control)
			array(), // Meta headers
