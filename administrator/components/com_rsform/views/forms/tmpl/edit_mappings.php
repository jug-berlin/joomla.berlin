		...
		<tbody>
			...
			<tr class="row<?php echo $k; ?>" style="cursor: move;">
				...
				<td align="center" width="20%" nowrap="nowrap">
					<button type="button" class="rs_button rs_left" onclick="openRSModal('<?php echo JRoute::_('index.php?option=com_rsform&view=mappings&cid='.$row->id.'&tmpl=component&formId='.$this->formId.'&cid='.$row->id); ?>', 'Mappings', '800x600')"><?php echo JText::_('RSFP_EDIT'); ?></button>
					<button type="button" class="rs_button rs_left" onclick="mappingdelete(<?php echo $this->formId; ?>,<?php echo $row->id; ?>);"><?php echo JText::_('RSFP_DELETE'); ?></button>
				</td>
			</tr>
			<?php $k=1-$k; ?>
			<?php $i++; ?>
