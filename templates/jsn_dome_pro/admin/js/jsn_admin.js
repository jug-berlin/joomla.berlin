...
var JSNAdmin = {
	...
	clean: function(){
		...
		{				
			empties.each(function(empty) {
				...
				if(empty.className.indexOf('jsn-about') == -1)
				{
	...
	displayUpdateBadge: function(text, version)
	{
		var parent = $$('#about-options .toggle-arrow .param-icon span');
		var span  = new Element('span', {'class': 'update-available', 'html':text + ' - ' + version});
		span.inject(parent[0]);
	}
};
