...
class RSFormModelDirectory extends JModelLegacy
{
	...
	public function getReplacements($user_id) {	
		...
		$replace = array('{global:sitename}', '{global:siteurl}', '{global:userid}', '{global:username}', '{global:email}', '{global:useremail}', '{global:fullname}');
		$with 	 = array($sitename, $siteurl, $user->id, $user->username, $user->email, $user->email, $user->name);
		...
		if ($static && $staticFields) {			
			...
			foreach ($staticFields as $field) {
				...
				if ($field == 'DateSubmitted') {
					$static[$field] = JFactory::getDate($static[$field], JFactory::getConfig()->get('offset'))->toSql();
				}
